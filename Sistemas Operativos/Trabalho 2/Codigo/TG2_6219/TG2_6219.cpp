...
#define ADVERSARIO_POSICAO_INICIAL 5
#define EMPTY _T( ' ' )
...
void movimentarAdversario(){
	//movimento com abertura de portas das salas do adversario
	int movimento = 0;

	movimento = rand() % 4;

	if (movimento == 0 && (mapa[adversario.adversarioPosicao].norte != -1)){
		adversario.adversarioPosicao = mapa[adversario.adversarioPosicao].norte;
	}
	if (movimento == 1 && (mapa[adversario.adversarioPosicao].sul != -1)){
		adversario.adversarioPosicao = mapa[adversario.adversarioPosicao].sul;
	}
	if (movimento == 2 && (mapa[adversario.adversarioPosicao].este != -1)){
		adversario.adversarioPosicao = mapa[adversario.adversarioPosicao].este;
	}
	if (movimento == 3 && (mapa[adversario.adversarioPosicao].oeste != -1)){
		adversario.adversarioPosicao = mapa[adversario.adversarioPosicao].oeste;
	}
}
...
void salvarJogo(struct Jogador *jogador, struct Adversario *adversario){
	FILE *f1; //ficheiro txt
	FILE *f2; //ficheiro bin

	f1 = fopen("..\\Debug\\data\\saveGame.txt", "w");
	f2 = fopen("..\\Debug\\data\\saveGame.bin", "wb");
	...
	//garda em .bin
	if (f2 != NULL){
		fwrite(jogador, sizeof(struct Jogador), 1, f2);
		fwrite(adversario, sizeof(struct Adversario), 1, f2);
	}
...
/*Secção das Threads. Aqui o processo é "automatizar" o movimento do adversario e combate entre as personagens*/
DWORD WINAPI ThreadProc1Adversario(LPVOID lpParam)
{
	while (adversario.adversarioEnergia > 0)
	{
		//Se o Jogador e o Monstro estiverem na mesma sala, o adversario fica bloqueado.
		WaitForSingleObject(hMutexAdversario, INFINITE);
		movimentarAdversario();
		ReleaseMutex(hMutexAdversario);
	}
...
DWORD WINAPI ThreadProc2Combate(LPVOID lpParam)
{
...
int main(int argc, char* argv[])
{
	...
	//cria as 2 threads: luta e adversario
	HANDLE hThread1Adversario = CreateThread(
		NULL,              // default security attributes
		0,                 // use default stack size  
		ThreadProc1Adversario,        // thread function 
		NULL,             // argument to thread function 
		0,                 // use default creation flags 
		NULL);   // returns the thread identifier 


	HANDLE hThread2Combate = CreateThread(
		NULL,              // default security attributes
		0,                 // use default stack size  
		ThreadProc2Combate,        // thread function 
		NULL,             // argument to thread function 
		0,                 // use default creation flags 
		NULL);   // returns the thread identifier 
	...
	WaitForSingleObject(hThread1Adversario, INFINITE);
	WaitForSingleObject(hThread2Combate, INFINITE);
	CloseHandle(hThread2Combate);
	CloseHandle(hThread1Adversario);
	CloseHandle(hMutexAdversario);
